...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $query = "INSERT INTO items_gradschool (img, name, descr, quantity, size, price) 
                VALUES (?, ?, ?, ?, ?, ?)";
    ...
    if ($stmt = mysqli_prepare($conn, $query)) {
        ...
        if (mysqli_stmt_execute($stmt)) {
            ...
            mysqli_close($conn);
            echo "<script>alert('Item added successfully!'); window.location='ims_itemsMASTERAL.php';</script>";
        } else {
